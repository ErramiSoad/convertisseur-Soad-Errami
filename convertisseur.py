...
def eur_to_usd(eur, taux=1.1):  # Taux fixe
    return eur * taux

def usd_to_eur(usd, taux=1.1):
    return usd / taux

# Exemple d'utilisation
if __name__ == "__main__":
    montant_eur = 100
    montant_usd = eur_to_usd(montant_eur)
    print(f"{montant_eur} EUR is equal to {montant_usd} USD")

    montant_usd = 110
    montant_eur = usd_to_eur(montant_usd)
    print(f"{montant_usd} USD is equal to {montant_eur} EUR")
